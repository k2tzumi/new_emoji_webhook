...
class JobBroker {
  public static readonly CACHE_KEY_PREFIX = "JobBroker#";
  public static readonly MAX_SLOT = 10;
  public static readonly DELAY_DURATION = 150;
  // 1 Hour
  public static readonly JOB_TIME_OUT = 3600000;
  ...
  public enqueue(callback: JobFunction, parameter: {}): void {
    ...
    const newTrigger: Trigger = ScriptApp.newTrigger(callback.name)
      .timeBased()
  ...
  public dequeue(
    handler: string
  ): { job: JobParameter; trigger: Trigger } | null {
        ...
        switch (state) {
          ...
          case "starting":
            ...
            if (new Date().getTime() - start_at > JobBroker.JOB_TIME_OUT) {
              ...
              console.info(
                `job time out. id: ${id}, handler: ${trigger.getHandlerFunction()}, created_at: ${created_at}, start_at: ${start_at}, parameter: ${parameter}`
              );
            }
            break;
          case "end":
  ...
  public consumeJob(closure: JobFunction): void {
    const pop: { job: JobParameter; trigger: Trigger } = this.dequeue(
      this.consumeJob.caller.name
    );
    ...
    if (pop) {
      ...
      console.info(
        `job starting. id: ${job.id}, created_at: ${job.created_at}, start_at: ${job.start_at}, parameter: ${job.parameter}`
      );
      ...
      try {
        ...
        this.saveJob(trigger, job);
        console.info(
          `job success. id: ${job.id}, created_at: ${job.created_at}, start_at: ${job.start_at}, start_at: ${job.end_at}, parameter: ${job.parameter}`
        );
      } catch (e) {
        job.state = "failed";
        job.end_at = new Date().getTime();
        this.saveJob(trigger, job);
        console.warn(
          `job failed. message: ${e.message}, stack: ${e.stack}, id: ${job.id}, created_at: ${job.created_at}, start_at: ${job.start_at}, start_at: ${job.end_at}, parameter: ${job.parameter}`
        );
      }
