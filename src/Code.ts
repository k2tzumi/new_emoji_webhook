...
function emojiChanged(event: string[]) {
  if (event['subtype'] === 'add') {
    var emoji_name = event["name"]
    console.log(emoji_name)
    var message = `${NOTIFICATION_MESSAGE} :${emoji_name}: \`:${emoji_name}:\``

    if (event['value'].indexOf('alias:') === 0) {
      var origin_emoji = event['value'].replace(/^alias:/, '')
...
function eventIdProceeded(eventId: string): boolean {
  var cash = CacheService.getScriptCache();
  var prevEventId = cash.get(eventId);
  if (prevEventId) {
    return true
  } else {
