...
abstract class SlackBaseHandler {
    ...
    protected listners: Map<String, Function>;

    public constructor(private verificationToken: string) {
        this.cache = CacheService.getScriptCache();
        this.listners = new Map<String, Function>();
    }

    abstract handle(e): { performed: boolean; output: TextOutput | null; }

    public addListener(type: string, handler: Function) {
        this.listners.set(type, handler);
    }

    protected getListener(type: string): Function | null {
